...
| 2.4 | Explicit declarations | Required in strict execution | Full declaration analysis not yet implemented | Recommendation |
...
| 5 | Input, output, arithmetic, relations, logic and standard functions | Supported | Parsed and standard callable case checked | Formal |
...
| 8 | Procedures, functions, `RETURN`, `BYVAL` and `BYREF` | Supported | Parsed | Formal |
...
| 10 | Classes, access control, constructors, inheritance and `SUPER` | Supported | Parsed | Formal |
...
The profile does not yet perform compiler-style semantic analysis. In
particular, it does not prove declaration or initialization, expression and
argument types, callable return paths, or reachability. It also does not
prescribe camelCase/PascalCase beyond ASCII and consistent case-insensitive
spelling. These gaps are separate from features that cannot be recovered from
plain text, such as font choice and wrapped continuation-line alignment.
